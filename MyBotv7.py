...
def selfdestruct_navigation(ship, game_map, avoid_moves, destination, myself):
    """Check cheaper way for navigation"""
    directions = []
    # logging.info("Ship position: {}".format(ship.position.x))
    ship_position = [ship.position.x, ship.position.y]
    ...
    if x_dist > 0 or (x_dist < - game_map.width // 2):
        new_position = ship.position.directional_offset(Direction.East)
        if (not game_map[new_position].is_occupied and not new_position in avoid_moves) or myself.shipyard.position == new_position:
            x_value = game_map[new_position].halite_amount
            directions.append(Direction.East)
        else:
            x_value = 9999
...
while True:
    ...
    for ship in me.get_ships():
        ...
        if self_destruct:
            ...
            if dropoff_count < max_dropoff and distance_to_base > 8 and game_map[ship.position].halite_amount > 200 and me.halite_amount >= constants.DROPOFF_COST:
